...
**第 1 步 · 了解现状**
运行 `npm run state`（只读摘要：各地块占用、建筑名册、空位建议、下一个建筑 id、各模型自建积木清单 `custom_blocks`、街区居民构成 `block_residents`——选址前对照它判定街区主权）。**不要读全城代码**：非本人、非官方的建筑与积木源码不开放查阅（宪法第 11 条「源码不外借」）——学形制看 preview 渲染与各建筑 NOTES.md，学代码格式看官方 `blocks/official/` 与 `lib/*`。

**第 2 步 · 选址与选题**
从 `free_lot_suggestions` 或规划图（`cities/c1/plan.json`，agent 只读）选空地块，并形成**立项提案**：建什么（类型与立意、与城市叙事的关系——呼应哪栋对景/补全市政哪块功能/契合街区主题）、在哪（地块号，对照 `block_residents` 判街区主权）、初步体量与三角预算档位。
...
**第 3 步 · 立项确认（城主门，宪法第 12 条）**
把立项提案呈城主（会话内明问明答——zcode 用 AskUserQuestion，其他 agent 用其等价确认机制），**获城主明确同意后方可写码动工**：城主提调整 → 改案重提；城主否决 → 另择题。确认实录（日期 + 结论，可引城主原话）写进 NOTES.md 的「立项」节——inspect R12 机械校验（2026-09-26 立法前已开工的在建建筑豁免）。城主未答复时停在提案态等待——宁停勿抢。

**第 4 步 · 方案比选**
在会话内构思三个形态差异明显的方案（体量/风格/细节策略各不同），比选后将结论与**三角预算分配表**写进 NOTES.md 的「设计」节，再进入第 5 步。本城鼓励一栋楼投入充足 token 与 30–60 分钟施工时间——**宁细勿糙**（R11 底线：三角形 ≥ 50,000、mesh ≥ 60；护栏 500,000 不是省钱目标——花得越满越好）。
...
| R12 | 设计文档：NOTES.md ≥ 200 字且含「预算」分配节；2026-09-26 立法后开工者另须含「立项」确认节（官方建筑豁免） |
...
- **R12 缺失/过短**：补 NOTES.md：立项确认实录、立意、三方案比选结论、形制分项、三角预算分配表。缺「立项」节（2026-09-26 后开工）→ 先停工，走宪法第 12 条立项确认，把城主确认实录（日期+结论）补进 NOTES。
