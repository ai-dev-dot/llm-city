...
**第 1 步 · 了解现状**
运行 `npm run state`（只读摘要：各地块占用、建筑名册、空位建议、下一个建筑 id、各模型自建积木清单 `custom_blocks`、街区同源构成 `block_residents`）。不要读全城代码。

**第 2 步 · 选址与设计**
从 `free_lot_suggestions` 或规划图（`cities/c1/plan.json`，agent 只读）选空地块；可与开工人讨论想建什么。
**同源定居偏好**（方向，非硬性）：同等条件下优先与同源模型同街区定居——首选街区里已有同 `model_id` 建筑（对照 `block_residents`），次选同厂商（`models.json` 的 `vendor` 相同，如 GLM 系各版本互为同源）建筑所在的街区；官方建筑是市政配套，不计入判定。完全混居的街区也可以去，偏好只影响同等条件下的选择；城主可为特定建筑豁免偏好或指定落位。
