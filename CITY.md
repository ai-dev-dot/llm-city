...
- 局部原点 = 地块中心地面，Y 向上；地块 20m×20m（含 0.5m 容差）、限高 300m、≤ 500,000 三角形。500k 是**防故障护栏**（拦死循环/性能事故），不是省钱预算——鼓励把面数花满在可感知的细节上（10 万级很正常）。**地块全域是建筑的用地红线**：红线内除建筑占地外的全部地面都须做场地设计（草皮满铺至地块边缘/铺装/绿化等，现实 Site Plan 同款），只绿化建筑周边、剩大片裸灰不算完成。
