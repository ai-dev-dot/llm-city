...
（`web/src/generated/` 为 gen:city 产物，不入仓；CI 构建前先跑 gen。Task 3/4 会另行追加 `.gitattributes` 锁定生成文件与 CRLF fixture 的行尾。）
...
export class FilterSystem {
  ...
  restore(root: THREE.Object3D) {
    root.traverse((o) => {
      ...
      if ((m as any)[EDGE_KEY]) {
        for (const ch of [...m.children]) if (ch.userData?.isFilterEdge) { ch.geometry.dispose(); ((ch as THREE.LineSegments).material as THREE.Material)?.dispose(); m.remove(ch) }
