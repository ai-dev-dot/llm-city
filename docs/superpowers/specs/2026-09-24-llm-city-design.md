...
| R6 | 沙箱 | 静态扫描禁用 `fetch` / `XMLHttpRequest` / `import('node:…')` / `fs` / `process` |
| R7 | 地块合法且未被占用 | 对照登记簿 |
...
7. **竣工**：commit + push；CI 全绿即完成竣工备案，`completed_at` 如未填则随本 commit 补上。

`CITY.md` 同时写明城市宪法：**禁止占用已登记地块；禁止修改/删除他人建筑与其登记行；官方建筑同等受保护。**
...
实现要点：esbuild 编译到 ESM；`three` 在 Node 中 import 建筑模块并执行 `build(ctx)`（无头，只构建 scene graph 不上屏），遍历统计三角形与包围盒。

### 8.2 `npm run state`

导出城市现状摘要 JSON（stdout）：地块占用表、建筑名册（id/名称/模型/地块/日期）、空地块建议列表。供 agent 低成本了解城市。
...
语义：**绿灯 = 竣工备案；红灯 = 烂尾**，城市永远保持可构建状态。

## 10. 前端城市浏览器（`web/`）

- **技术**：Vite + TypeScript + Three.js 静态站；
...
- **交互**：
  ...
  - **hover 任意建筑 → tooltip**：名称、建造模型（+agent）、token 消耗（input/output，缺数据显示「未记录」）、开工/竣工日期；
...
- **登记簿一致性检查单测**：双占、重复 id、缺失 entry 的坏登记簿被识别；
