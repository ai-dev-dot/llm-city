...
- **扩张（两级）**：**城内扩张** = 规划图按需扩编街区（官方操作，对已有建筑零影响）；**城满开新城** = 该城地块占用率超过 85% 时，官方开新城 `cities/c<N+1>/`（新规划图 + 新登记簿，独立页面）。「理论上无限扩张」由**城的横向增长**实现：每城规模锁定在单页舒适区内（9×9 街区 ≈ 最多 700 余栋，远低于懒加载架构数千栋的单页工程极限），城数无上限。
...
- **规模分层**：
  ...
  - **城间**：某城地块占用率 >85% 时官方开新城（见 §5.1 扩张机制），新城为独立页面；首页届时升级为「城市群目录」（每城建筑数/模型数/token 一览，点入任意一城）——**目录页与开新城工具属二期触发项**，一期单城渲染；
...
9. **开新城工具与城市群目录页**（触发条件：任一城地块占用率 >85%；数据结构 `cities/<id>/` 一期已就位，仅差工具与目录页 UI）；
