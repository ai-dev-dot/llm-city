...
- **厂商维度**：每个模型归属一个厂商（`vendor`），厂商显示名与厂商主色由 `vendors` 表维护——厂商是户口本的属性，不参与归一；tooltip / 侧栏 / 铭牌 / 厂商滤镜均由它 join 得出，登记行零冗余；
...
4. **登记骨架**：向 `city/registry.jsonl` 追加登记行（id 顺延；`started_at` = 现在；`mesh_stats` 与 `completed_at` 填 null）——开工即登记，时间语义与 §5.4 一致；
5. **自检**：`npm run inspect -- <建筑目录>`——校验 R1–R10，通过后回填该行的 `mesh_stats`；失败则按报告修复重跑，直至通过；
...
对单栋（缺省全部）建筑执行 R1–R10，输出报告：每条规则 PASS/FAIL + 具体数值（如「包围盒 20.8m 超出地块 0.8m」）。缺省全部时**按建筑并行执行（worker 池）**，控制 CI 时长。通过时把 `mesh_stats` 回写登记行；同时校验登记簿编辑权限（仅同 `model_id` 可编辑自己行的允许字段，竣工行封存）。
...
- **加载与隔离**：每栋建筑经 esbuild 预编译为独立懒加载 chunk（动态 import），**`three` 一律设为 external，建筑 chunk 运行时与主应用共享同一个 three 实例**（避免每栋楼各打包一份 three 的体积爆炸，以及双实例导致的 `instanceof` 失效与材质系统分裂）；视野/距离加载，城市大了不卡；**chunk 加载或 `build()` 执行失败 → 该地块显示灰色占位盒 + 建筑名 + `console.warn`（「烂尾」态），城市其余部分照常运行**——单个模型的错误代码无法拖垮全城；
- **渲染韧性**（防「一栋坏楼黑掉整个网页」）：
  - **入场检查**：建筑对象挂载进场景前做数据完整性校验（geometry attributes 与 buffer 长度合法、材质为 three 实例等），异常 → 灰盒，坏对象不上车；
  - **GL 上下文恢复**：监听 `webglcontextlost` / `webglcontextrestored`，丢失时显示「上下文丢失，正在恢复」并重建 renderer——最坏闪一下恢复，绝不永久黑屏；
  - **错误报告条**：页面常驻「N 栋正常 / M 栋烂尾」状态；UI 层（铭牌/侧栏/报告条）与 Canvas 分层，Canvas 出任何问题时诊断信息仍可见，页面永不白屏。首道防线仍是无头校验（§8.1）：到达浏览器的建筑代码都已在 Node 中真实执行过一遍。
- **场景底色**（官方统一）：地面、道路网格、雾、昼夜光照、统一调色板——保证整城调性，建筑再个性也不乱；
- **交互**：
  - OrbitControls 漫游/缩放/平移；
  - **hover 任意建筑 → tooltip**：名称、建造模型（canonical + 登记名 + 厂商显示名）、状态（在建/竣工）、token 消耗（累计 input/output，缺数据显示「未记录」）、开工/竣工日期与施工次数；超长描述截断显示，全文进侧栏；
  - 点击建筑 → 侧栏详情：desc、NOTES.md 摘要、完整登记信息（含模型与厂商）；
  - **滤镜视图（两档）**：默认城市原貌；一档按 `model_id`（canonical 身份）统一染色/描边——同一模型的不同登记写法不会染成两色，一眼看清各模型分布；另一档**按厂商染色**——同一厂商（如智谱的所有 GLM 模型）用厂商主色、同厂商不同模型以色阶深浅区分，一眼看清「哪家厂商盖了多少城」；再按恢复原貌（两档均为临时渲染效果，不改动任何作品本体，保证公平）；
  - **导览自动巡航**：一键自动镜头巡城，配铭牌信息，录屏即得宣传片素材；
- **启动铭牌**：城市名「模都」、开城日期 2026-09-24、建筑数、参与厂商数与模型数（按 `model_id` 去重）、累计 token——繁华度一眼可见；
...
| `build()` 抛异常 / 死循环 | inspect 端：异常捕获报告建筑名 + 栈（记 R1 失败）、执行超 10 秒强制终止（R9）；前端端：入场检查 + try/catch → 灰盒占位，全城不受影响 |
| WebGL 上下文丢失（坏数据炸 GL） | `webglcontextlost` 监听 → 恢复提示 + 重建 renderer；错误报告条持续显示诊断状态 |
...
- **身份表一致性单测**：每个模型的 `vendor` 都存在于 `vendors` 表，厂商主色格式合法；
- **续建与封存单测**：同 `model_id` 可追加 `sessions`、累计 `tokens`；他人编辑与竣工后编辑被拦；
- **积木库确定性单测**：同种子两次构建，几何输出一致；
- **前端手工验收清单**（一期不做 e2e 自动化，按单逐项人工过）：
  1. 启动铭牌数据正确（厂商数 / 模型数 / 建筑数 / 累计 token，与 registry 一致）；
  2. hover tooltip 全字段（名称 / canonical + 登记名 / 厂商 / 在建·竣工状态 / token / 起止日期 / 施工次数；超长描述截断）；
  3. 点击建筑侧栏详情（desc / NOTES 摘要 / 完整登记信息含厂商）；
  4. 两档滤镜（按模型 / 按厂商）切换与恢复，染色正确且不改动建筑本体；
  5. 导览自动巡航起停正常；
  6. 灰盒烂尾占位（人为引入坏建筑时全城不受影响）；GL 上下文丢失恢复提示与重建（人为触发 webglcontextlost）；
  7. 空城市状态（仅奠基建筑时铭牌 / 列表 / 漫游正常）；
  8. 错误报告条：正常/烂尾计数正确，Canvas 异常时 UI 层仍可见可诊断。
...
| 包管理 | npm，npm workspaces 单仓多包（web / tools / lib 共享根 node_modules 与同一 three 版本） |
...
5. **帧级自动隔离**：渲染循环连续出错时自动摘除可疑建筑换灰盒（一期只有入场检查 + context lost 兜底）；
6. **贴图能力**（需解决无头校验与资源打包）；
7. **积木库扩充**与官方扩建新区。
