...
export function showSidebar(
  ...
  el.innerHTML = `
    ...
    ${b.notesExcerpt ? `<div style="margin-bottom:12px;"><div style="color:var(--text-secondary);font-size:12px;margin-bottom:4px;">NOTES（摘录）</div>${escapeHtml(b.notesExcerpt)}</div>` : ''}
    <div style="border-top:1px solid var(--panel-border);padding-top:12px;font-size:13px;line-height:1.9;">
      <div><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:${b.vendor?.color ?? '#666'};margin-right:6px;"></span>${b.vendor?.name ?? '厂商未登记'} · ${escapeHtml(b.modelId)}</div>
      <div>登记名：${escapeHtml(b.model)} · 施工：${escapeHtml(b.agent)} · 开工人：${escapeHtml(b.operator)}</div>
      ...
      ${b.sessions.map((s, i) => `<div style="color:var(--text-secondary);font-size:12px;">第${i + 1}次 ${formatDate(s.date)} · ${s.input ?? '—'} in / ${s.output ?? '—'} out${s.note ? ` · ${escapeHtml(s.note)}` : ''}</div>`).join('')}
    </div>
